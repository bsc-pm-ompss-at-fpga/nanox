   ...
   */
   ...
   class RegionDirectory
   {
      ...
      private:
         ...
         uint64_t _getKey( uint64_t addr );
         void _unregisterObjects( std::map< uint64_t, HashBucket * > &objects );
         void _invalidateObjectsFromDevices( std::map< uint64_t, HashBucket * > &objects );
