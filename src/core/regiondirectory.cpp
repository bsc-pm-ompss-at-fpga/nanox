...
memory::Address RegionDirectory::_getKey( memory::Address addr ) const {
...
GlobalRegionDictionary *RegionDirectory::getRegionDictionary( memory::Address objectAddr, bool canFail ) {
#if 0
   unsigned int key = ( jen_hash( objectAddr ) & (HASH_BUCKETS-1) );
#else
...
void RegionDirectory::_invalidateObjectsFromDevices( std::map< memory::Address, MemoryMap< Object > * > &objects ) {
...
void RegionDirectory::_unregisterObjects( std::map< memory::Address, MemoryMap< Object > * > &objects ) {
   for ( std::map< memory::Address, MemoryMap< Object > * >::iterator it = objects.begin(); it != objects.end(); it++ ) {
      Object *o = it->second->getExactByAddress(it->first);
...
void RegionDirectory::synchronize( WD &wd, memory::Address addr ) {
...
void RegionDirectory::synchronize( WD &wd, size_t numDataAccesses, DataAccess *data ) {
   //std::ostream &o = (*myThread->_file);
   ...
   for ( std::size_t idx = 0; idx < numDataAccesses; idx += 1 ) {
      ...
      memory::Address objectAddr = data[idx].getDepAddress(); //NOTE: Using the region base address in the host
      GlobalRegionDictionary *dict = getRegionDictionary( objectAddr, true );
...
void RegionDirectory::unregisterObject( memory::Address baseAddr) {
