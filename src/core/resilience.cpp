...
namespace nanos {
      ...
      , _resilienceResultsFilepath( NULL )
    {
        ...
        if( resilienceResultsFileSize == 0 ) { 
            ...
            resilienceResultsFileSize = 1024*1024*512;/*sizeof(ResilienceNode);*/
        }
        ...
        _RESILIENCE_RESULTS_MAX_FILE_SIZE += 4*sizeof(unsigned long);
        ...
        if( !_restore ) {
            ...
            init[1] = 0; 
            init[2] = 0;
            init[3] = 4*sizeof(unsigned long);
            ResultsNode * results = ( ResultsNode * ) getResilienceResults( init[3] );
            results->free_info.next_free = 0;
            results->free_info.size = _RESILIENCE_RESULTS_MAX_FILE_SIZE - 4*sizeof(unsigned long);
        }
    ...
    ResiliencePersistence::~ResiliencePersistence() {
        ...
        int res = munmap( _resilienceResults, _RESILIENCE_RESULTS_MAX_FILE_SIZE + 4*sizeof(unsigned long) );
        if( res == -1 )
            fatal( "Error unmapping file." );
        close( _resilienceResultsFileDescriptor );
    ...
    void ResilienceNode::storeInput( CopyData * copies, size_t numCopies, int task_id ) {
        ...
        if( dataSize == 0 ) {
            ...
            if( inputs_size <= 0 )
                return;
            dataSize = inputs_size;
        }

        //Reserve _dataSize bytes in results.
        if( dataIndex == 0 )
            dataIndex = sys.getResiliencePersistence()->getResilienceResultsFreeSpace( dataSize );

        //Copy the result
        char * aux = ( char * ) sys.getResiliencePersistence()->getResilienceResults( dataIndex );
        if( aux == NULL )
        ...
        unsigned long * init = ( unsigned long * ) _resilienceResults;
        unsigned long old_checkpoint_id = init[0];
        if( old_checkpoint_id != 0 ) {
            index_to_free = init[1];
            size_to_free = init[2];
        }
        ...
        // TODO: FIXME: Maybe this update needs a lock. 
        init[0] = id;
        init[1] = dataIndex;
        init[2] = dataSize;
    ...
    void ResiliencePersistence::loadData( CopyData * copies, size_t numCopies ) { 
        ...
        unsigned long * init = ( unsigned long * ) _resilienceResults;
        unsigned long dataIndex = init[1];
        char * aux = ( char * ) sys.getResiliencePersistence()->getResilienceResults( dataIndex );
        for( unsigned int i = 0; i < numCopies; i++ ) {
            if( copies[i].isInput() ) {
                size_t copy_size = copies[i].getDimensions()->accessed_length;
