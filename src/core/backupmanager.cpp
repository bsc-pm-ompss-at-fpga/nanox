...
BackupManager::BackupManager ( const BackupManager &arch ) : Device ( arch ), _memsize(arch._memsize), _managed_pool(arch._managed_pool) {}

BackupManager::~BackupManager(){}

//debe ser privado
const BackupManager & BackupManager::operator= ( const BackupManager &arch ){
...
void BackupManager::*memAllocate ( size_t size, SeparateMemoryAddressSpace &mem,
                    uint64_t targetHostAddr = 0 ) const {
   _managed_pool.allocate(size);

}

void BackupManager::memFree ( uint64_t addr, SeparateMemoryAddressSpace &mem ) const {
