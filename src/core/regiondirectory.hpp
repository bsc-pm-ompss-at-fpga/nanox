...
inline void DirectoryEntryData::setBaseAddress(memory::Address addr) {
   _baseAddress = addr;
}

inline memory::Address DirectoryEntryData::getBaseAddress() const {
   return _baseAddress;
}
...
inline RegionDirectory::RegionDirectoryKey RegionDirectory::getRegionDirectoryKey( memory::Address addr ) {
