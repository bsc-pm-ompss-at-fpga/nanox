   ...
   if ( cd.getNumDimensions() != this->getNumDimensions() ) {
      ...
      for ( unsigned int cidx = 0; cidx < cd.getNumDimensions(); cidx += 1 ) {
         if ( this->getDimensionSizes()[ cidx ] != cd.getDimensions()[ cidx ].size ) {
            fatal( "Object with base address ", std::hex, cd.getBaseAddress(),
                   " was previously registered with a different size in dimension ", std::dec, cidx,
                   " (previously was ", std::dec, this->getDimensionSizes()[ cidx ],
                   " now received size ", std::dec, cd.getDimensions()[ cidx ].size, ")."
                 );
         }
      }
