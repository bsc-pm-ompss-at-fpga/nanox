...
constexpr typename enable_if<is_frequency<ToFrequency>::value,ToFrequency>::type
frequency_cast( frequency<Rep,Resolution> const& f )
{
	...
	typedef ratio_divide<Resolution, to_resolution> cf;
	typedef typename common_type<to_rep, Rep, intmax_t>::type cr;
...
struct _frequency_cast_impl
{
   ...
   static constexpr operator ToFreq (const duration<Rep, Resolution>& d)
   {
