...
class SignalInfo {
	...
	public:
		...
		int getSignalNumber() const { return savedInfo.si_signo; }

		int getErrorNumber() const { return savedInfo.si_errno; }

		int getSignalCode() const { return savedInfo.si_code; }

		Address getAddress() const { return Address( savedInfo.si_addr ); }
		...
		{
			return MemoryChunk( 
						Address( savedInfo.si_addr ).alignToLSB( savedInfo.si_addr_lsb ),
						1<<savedInfo.si_addr_lsb
					);
		}
};
